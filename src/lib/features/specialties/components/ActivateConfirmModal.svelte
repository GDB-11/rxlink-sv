<!-- src/lib/features/specialties/components/ActivateConfirmModal.svelte -->
<script lang="ts">
    interface Props {
        open: boolean;
    ...
    let { open, specialtyName, submitting, error, onconfirm, oncancel }: Props = $props();
</script>
        ...
        <div class="w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-xl dark:bg-stone-800">
            ...
            <div class="px-6 py-5">
                ...
                <p id="confirm-desc" class="mt-2 text-sm text-stone-600 dark:text-stone-400">
                    ¿Desea activar
                    <strong class="font-medium text-stone-900 dark:text-stone-100">{specialtyName}</strong>?
                    La especialidad volverá a estar disponible en el catálogo.
                </p>
