<!-- src/lib/features/patients/components/DeactivatePatientModal.svelte -->
<script lang="ts">
    interface Props {
        open: boolean;
        patientName: string;
    ...
    >
        <div class="w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-xl dark:bg-stone-800">
            <div class="border-b border-stone-200 px-6 py-4 dark:border-stone-700">
                <h2 id="deactivate-patient-title" class="text-base font-semibold text-stone-900 dark:text-stone-50">
                    Desactivar paciente
                </h2>
            </div>
            <div class="px-6 py-5">
                ...
                <p class="text-sm text-stone-600 dark:text-stone-400">
                    ¿Desactivar a <strong class="text-stone-900 dark:text-stone-50">{patientName}</strong>?
                    El registro se conserva y puede reactivarse.
                </p>
            </div>
            <div class="flex justify-end gap-3 border-t border-stone-200 px-6 py-4 dark:border-stone-700">
                ...
                <button
                    ...
                    onclick={onconfirm}
                    disabled={submitting}
                    class="cursor-pointer rounded-lg bg-rose-500 px-4 py-2 text-sm font-medium text-white
                           transition-colors hover:bg-rose-600
                           disabled:cursor-not-allowed disabled:opacity-50"
                >
