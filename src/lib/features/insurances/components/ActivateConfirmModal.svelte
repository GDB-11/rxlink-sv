<!-- src/lib/features/insurances/components/ActivateConfirmModal.svelte -->
<script lang="ts">
    interface Props {
        open: boolean;
    ...
    let { open, insuranceName, submitting, error, onconfirm, oncancel }: Props = $props();
</script>
        ...
        <div class="w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-xl dark:bg-stone-800">
            ...
            <div class="px-6 py-5">
                ...
                <p id="confirm-desc" class="mt-2 text-sm text-stone-600 dark:text-stone-400">
                    ¿Desea activar
                    <strong class="font-medium text-stone-900 dark:text-stone-100">{insuranceName}</strong>?
                    El seguro volverá a estar disponible al registrar pagos.
                </p>
