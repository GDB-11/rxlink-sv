<!-- src/lib/features/users/components/ActivateUserModal.svelte -->
<script lang="ts">
    interface Props {
        open: boolean;
        userName: string;
    ...
    let { open, userName, submitting, error, onconfirm, oncancel }: Props = $props();
</script>
...
{#if open}
    <div
        ...
        aria-describedby="confirm-activate-desc"
    >
        <div class="w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-xl dark:bg-stone-800">

            <div class="px-6 py-5">
                <h2 id="confirm-activate-title" class="text-base font-semibold text-stone-900 dark:text-stone-50">
                    Activar usuario
                </h2>
                <p id="confirm-activate-desc" class="mt-2 text-sm text-stone-600 dark:text-stone-400">
                    ¿Desea activar a
                    <strong class="font-medium text-stone-900 dark:text-stone-100">{userName}</strong>?
