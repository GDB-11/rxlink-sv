<!-- src/lib/features/users/components/DeactivateUserModal.svelte -->
<script lang="ts">
    interface Props {
        open: boolean;
        userName: string;
...
{#if open}
    ...
    >
        <div class="w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-xl dark:bg-stone-800">
            ...
            <div class="px-6 py-5">
                <h2 id="confirm-deactivate-title" class="text-base font-semibold text-stone-900 dark:text-stone-50">
                    Desactivar usuario
                </h2>
                <p id="confirm-deactivate-desc" class="mt-2 text-sm text-stone-600 dark:text-stone-400">
                    ¿Desea desactivar a
